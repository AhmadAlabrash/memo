...
import '../../model/auth/email_resend_code_model.dart';
...
class AuthApiServices {
  ...
  static Future<EmailResendCodeModel?> emailResendProcess() async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: false).get(
        "${ApiEndpoint.registerOtpResendURL}${LocalStorages.getEmailToken()} ",
        code: 200,
      );
      if (mapResponse != null) {
        EmailResendCodeModel result =
            EmailResendCodeModel.fromJson(mapResponse);
        return result;
      }
    } catch (e) {
      log.e('🐞🐞🐞 err from email resend code api ==> $e 🐞🐞🐞');
      CustomSnackBar.error('Something went Wrong!');
      return null;
    }
    return null;
  ...
  ///* Delete process process api
  static Future<CommonSuccessModel?> deleteApi(
      {required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: false).post(
      ...
      log.e(
          '🐞🐞🐞 err from delete process api service ==> $e 🐞🐞🐞');
      CustomSnackBar.error(
    ...
    ///* Log Out process process api
  static Future<CommonSuccessModel?> signOutApi(
      {required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: false).post(
      ...
      log.e(
          '🐞🐞🐞 err from log out process api service ==> $e 🐞🐞🐞');
      CustomSnackBar.error('Something went Wrong!');
      return null;
    }
    return null;
