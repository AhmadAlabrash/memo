...
const String emailOtpTokenKey = "emailOtpTokenKey";
...
class LocalStorages {
  ...
  static Future<void> saveName({required String name}) async {
    ...
    static Future<void> saveTrackingTrx({required String trx}) async {
    final box = GetStorage();

    await box.write(trackingKey, trx);
  ...
  static Future<void> saveToken({required String token}) async {
    ...
    static Future<void> saveEmailToken({required String emailToken}) async {
    final box = GetStorage();

    await box.write(emailOtpTokenKey, emailToken);
  }
  ...
  static Future<void> saveTrxKey({required String trx}) async {
    final box = GetStorage();

    await box.write(trxKey, trx);
  }
  ...
  static String getTrx() {
    return GetStorage().read(trxKey) ?? 'Null';
  }
  ...
  static String getTrackingTrx() {
    return GetStorage().read(trackingKey) ?? '';
  }
  ...
  static String? getEmailToken() {
    var rtrn = GetStorage().read(emailOtpTokenKey);

    debugPrint(rtrn == null ? "## email Token is null###" : "");
  ...
  static Future<void> removeTrxValue() async {
    final box = GetStorage();
    await box.remove(trxKey);
    await box.remove(trackingKey);
